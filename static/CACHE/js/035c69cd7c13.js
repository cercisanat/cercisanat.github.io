...
$.cookie(key,'',$.extend({},options,{expires:-1}));return!$.cookie(key);};}));!function($){"use strict";$(function(){$.support.transition=(function(){var transitionEnd=(function(){var el=document.createElement('bootstrap'),transEndEventNames={'WebkitTransition':'webkitTransitionEnd','MozTransition':'transitionend','OTransition':'oTransitionEnd otransitionend','transition':'transitionend'},name
...
event.type='swipe';args.unshift(event,deltaX,deltaY);return($.event.dispatch||$.event.handle).apply(that,args);}}};!function(a){"function"==typeof define&&define.amd?define(["jquery"],a):"object"==typeof exports?module.exports=a:a(jQuery)}(function(a){function b(b){var g=b||window.event,h=i.call(arguments,1),j=0,l=0,m=0,n=0,o=0,p=0;if(b=a.event.fix(g),b.type="mousewheel","detail"in g&&(m=-1*g.detail),"wheelDelta"in g&&(m=g.wheelDelta),"wheelDeltaY"in g&&(m=g.wheelDeltaY),"wheelDeltaX"in g&&(l=-1*g.wheelDeltaX),"axis"in g&&g.axis===g.HORIZONTAL_AXIS&&(l=-1*m,m=0),j=0===m?l:m,"deltaY"in g&&(m=-1*g.deltaY,j=m),"deltaX"in g&&(l=g.deltaX,0===m&&(j=-1*l)),0!==m||0!==l){if(1===g.deltaMode){var q=a.data(this,"mousewheel-line-height");j*=q,m*=q,l*=q}else if(2===g.deltaMode){var r=a.data(this,"mousewheel-page-height");j*=r,m*=r,l*=r}if(n=Math.max(Math.abs(m),Math.abs(l)),(!f||f>n)&&(f=n,d(g,n)&&(f/=40)),d(g,n)&&(j/=40,l/=40,m/=40),j=Math[j>=1?"floor":"ceil"](j/f),l=Math[l>=1?"floor":"ceil"](l/f),m=Math[m>=1?"floor":"ceil"](m/f),k.settings.normalizeOffset&&this.getBoundingClientRect){var s=this.getBoundingClientRect();o=b.clientX-s.left,p=b.clientY-s.top}return b.deltaX=l,b.deltaY=m,b.deltaFactor=f,b.offsetX=o,b.offsetY=p,b.deltaMode=0,h.unshift(b,j,l,m),e&&clearTimeout(e),e=setTimeout(c,200),(a.event.dispatch||a.event.handle).apply(this,h)}}function c(){f=null}function d(a,b){return k.settings.adjustOldDeltas&&"mousewheel"===a.type&&b%120===0}var e,f,g=["wheel","mousewheel","DOMMouseScroll","MozMousePixelScroll"],h="onwheel"in document||document.documentMode>=9?["wheel"]:["mousewheel","DomMouseScroll","MozMousePixelScroll"],i=Array.prototype.slice;if(a.event.fixHooks)for(var j=g.length;j;)a.event.fixHooks[g[--j]]=a.event.mouseHooks;var k=a.event.special.mousewheel={version:"3.1.11",setup:function(){if(this.addEventListener)for(var c=h.length;c;)this.addEventListener(h[--c],b,!1);else this.onmousewheel=b;a.data(this,"mousewheel-line-height",k.getLineHeight(this)),a.data(this,"mousewheel-page-height",k.getPageHeight(this))},teardown:function(){if(this.removeEventListener)for(var c=h.length;c;)this.removeEventListener(h[--c],b,!1);else this.onmousewheel=null;a.removeData(this,"mousewheel-line-height"),a.removeData(this,"mousewheel-page-height")},getLineHeight:function(b){var c=a(b)["offsetParent"in a.fn?"offsetParent":"parent"]();return c.length||(c=a("body")),parseInt(c.css("fontSize"),10)},getPageHeight:function(b){return a(b).height()},settings:{adjustOldDeltas:!0,normalizeOffset:!0}};a.fn.extend({mousewheel:function(a){return a?this.bind("mousewheel",a):this.trigger("mousewheel")},unmousewheel:function(a){return this.unbind("mousewheel",a)}})});$(doc).ready(prepareDOM);window._photobox={history:history,defaults:defaults};})(jQuery,document,window);(function($){function inputs(form){return form.find(":input:visible:not(:button)");}
$.fn.validate=function(settings){settings=$.extend({type:'ul',callback:false,fields:false,event:'submit',submitHandler:null,fieldSuccessCallback:null,formSuccessCallback:null,fieldInvalidCallback:null,callback:null},settings);function ajaxForm(form,data,field,formSuccessCallback){var $form=$(form);var field=field||false;var formSuccessCallback=formSuccessCallback||false;$.ajax({async:false,data:data,dataType:'json',traditional:true,error:function(XHR,textStatus,errorThrown){status=true;},success:function(data,textStatus){status=data.valid;if(data.valid){if(settings.fieldSuccessCallback){settings.fieldSuccessCallback($('#'+field));}
$form.find('ul.errorlist').remove();if(formSuccessCallback){formSuccessCallback($form);}}else{if(settings.callback){settings.callback($form,data);}
else{var get_form_error_position=function(key){key=key||'__all__';if(key=='__all__'){var filter=':first';}else{var filter=':first[id^=id_'+key.replace('__all__','')+']';}
return inputs(form).filter(filter).parent();};if(settings.type=='p'){if(!data.errors[field]&&settings.fieldSuccessCallback){$('#'+field).parent().prev('ul.errorlist').remove();settings.fieldSuccessCallback($('#'+field));}
$.each(data.errors,function(key,val){if(key.indexOf('__all__')>=0){var error=get_form_error_position(key);if(error.prev().is('ul.errorlist')){error.prev().before('<ul class="errorlist"><li>'+val+'</li></ul>');}}
else{if(field){if(field==key&&$('#'+field).val().length>0){if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+field).parent().prev('ul.errorlist').remove();$('#'+key).parent().before('<ul class="errorlist"><li>'+val+'</li></ul>');}}else{if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+field).parent().prev('ul.errorlist').remove();$('#'+key).parent().before('<ul class="errorlist"><li>'+val+'</li></ul>');}}});}
if(settings.type=='table'){if(!data.errors[field]&&settings.fieldSuccessCallback){$('#'+field).siblings("ul.errorlist").remove();settings.fieldSuccessCallback($('#'+field));}
$.each(data.errors,function(key,val){if(key.indexOf('__all__')>=0){get_form_error_position(key).parent().before('<tr><td colspan="2"><ul class="errorlist"><li>'+val+'.</li></ul></td></tr>');}
else{if(field){if(field==key&&$('#'+field).val().length>0){if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+field).siblings("ul.errorlist").remove();$('#'+key).before('<ul class="errorlist"><li>'+val+'</li></ul>');}}else{if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+field).siblings("ul.errorlist").remove();$('#'+key).before('<ul class="errorlist"><li>'+val+'</li></ul>');}}});}
if(settings.type=='ul'){if(!data.errors[field]&&settings.fieldSuccessCallback){$('#'+field).siblings('ul.errorlist').remove();settings.fieldSuccessCallback($('#'+field));}
$.each(data.errors,function(key,val){if(key.indexOf('__all__')>=0){get_form_error_position(key).before('<li><ul class="errorlist"><li>'+val+'</li></ul></li>');}
else{if(field){if(field==key&&$('#'+field).val().length>0){if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+field).siblings('ul.errorlist').remove();$('#'+key).prev().before('<ul class="errorlist"><li>'+val+'</li></ul>');}}else{if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+key).prev().before('<ul class="errorlist"><li>'+val+'</li></ul>');}}});}
if(settings.type=='bootstrap'){if(!data.errors[field]&&settings.fieldSuccessCallback){$('#'+field).parent().prev('ul.errorlist').remove();settings.fieldSuccessCallback($('#'+field));}
$.each(data.errors,function(key,val){if(key.indexOf('__all__')>=0){var error=get_form_error_position(key);if(error.prev().is('ul.errorlist')){error.prev().before('<ul class="errorlist"><li>'+val+'</li></ul>');}}
else{if(field){if(field==key&&$('#'+field).val().length>0){if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+field).closest('.controls').addClass('error');$('#'+field).parent().find('ul.errorlist').remove();$('#'+key).parent().append('<ul class="errorlist"><li>'+val+'</li></ul>');}}else{if(settings.fieldInvalidCallback){settings.fieldInvalidCallback($('#'+field));}
$('#'+field).closest('.controls').addClass('error');$('#'+field).parent().find('ul.errorlist').remove();$('#'+key).parent().append('<ul class="errorlist"><li>'+val+'</li></ul>');}}});}}}},type:'POST',url:$form.attr('action')});}
if(settings.event!='submit'){if(Object.prototype.toString.call(settings.fields)=='[object Array]'){var $form=$(this);$(settings.fields).each(function(i,el){$('#'+el).each(function(index,el){$(el).on(settings.event,function(event){field=$(this).attr("id");ajaxForm($form,$form.serialize(),field,null);});});});}else{var $form=$(this);$('input, checkbox, select, textarea',this).not('input[type=submit]').each(function(index,el){$(el).on(settings.event,function(event){field=$(this).attr("id");ajaxForm($form,$form.serialize(),field,null);});});}}
$(this).on("submit",function(event){event.preventDefault();$(this).find('ul.errorlist').remove();ajaxForm($(this),$(this).serialize()+"&submit=true",null,settings.formSuccessCallback);});};})(jQuery);$(document).ready(function(){var ShowMenu=function(exclude){$('.starter-target:not('+exclude+')').removeClass('active');$(exclude).toggleClass('active');}
$('.menu-starter').click(function(){ShowMenu('header .primary-menu');});$('.genres-starter').click(function(){ShowMenu('nav.genres');});$('.search-starter').click(function(){ShowMenu('.form-search');});});$(document).ready(function(){$('nav .categories .genre').tooltip();$('.galleryitem').tooltip();});$(document).ready(function(){var notifications_trigger=$('.notifications-trigger');var notifications=$('.notifications');$('.more').tooltip();$('.less').tooltip();$('.social-accounts a').tooltip();$('.carousel').carousel('pause');if(document.body.clientWidth>767&&!$.cookie('animated')){notifications_trigger.animate({'height':'120px'},700).delay(1000).animate({'font-size':'2em'},200).animate({'font-size':'1em'},200).animate({'font-size':'2em'},200).animate({'font-size':'1em'},200).delay(1000).animate({'height':'35px'},700);setTimeout(function(){notifications_trigger.removeAttr("style")},4800);$.cookie('animated',true);}
if(document.body.clientWidth<=767&&!$.cookie('animated')){notifications_trigger.animate({'font-size':'2em'},200).animate({'font-size':'1em'},200).animate({'font-size':'2em'},200).animate({'font-size':'1em'},200);setTimeout(function(){notifications_trigger.removeAttr("style")},1000);$.cookie('animated',true);}
if($.cookie('subscribed_to_newsletters')){$('.subscribe-form').hide();$('.subscribe-success',notifications).show();}
notifications_trigger.on('click',function(){if(notifications.is(":visible")){notifications.slideUp('slow');$('.notifications-trigger .icon-bell').show();$('.notifications-trigger .icon-remove').hide();}
else{notifications.slideDown('slow');$('.notifications-trigger .icon-bell').hide();$('.notifications-trigger .icon-remove').show();}});$('.subscribe-form').validate({type:'bootstrap',fields:['id_email','id_name'],event:'focusout',fieldSuccessCallback:function(field){field.closest('.controls').addClass('ok');},formSuccessCallback:function(form){$.cookie("subscribed_to_newsletters",1,{expires:999999});form.hide();$('.subscribe-success',notifications).show();}});$('.another-subscription').on('click',function(e){$('.subscribe-success',notifications).hide();$('.subscribe-form').show();$('#id_email').val('');$('#id_email').closest('.controls').removeClass('ok');$('#id_email').closest('.controls').removeClass('error');$('#id_name').val('');$('#id_name').closest('.controls').removeClass('ok');$('#id_name').closest('.controls').removeClass('error');e.preventDefault();});});